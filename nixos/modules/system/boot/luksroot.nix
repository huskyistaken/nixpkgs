...
let
  ...
  in ''
    ...
    open_with_file() {
      if wait_target "key file" ${dev.cryptkeyDevice}; then
          # First mount the drive
          mount -t ${dev.cryptkeyFstype} ${dev.cryptkeyDevice} /crypt-mount
          cs_status=$?
          if [ $cs_status -ne 0 ]; then
          ...
          ${csopen} --key-file=/crypt-mount/${dev.cryptkeyPath}
          cs_status=$?
          if [ $cs_status -ne 0 ]; then
